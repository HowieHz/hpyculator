...
a3.4.1 创建了两个文件夹专门用来储存设置和输出,扩大1.5倍了默认生成的窗口
a3.4.0 在输入栏输入update_log点击运算就可以看更新日志了!菜单栏它来了
a3.3.4 修改保存名的时间
a3.3.3b 走向规范化
#a3.3.3a 遗弃的分支，消除了保存时间
a3.3.2 删除多余代码优化体积
a3.3.1 生成在gui里的不再是一行了,砍掉了可以修改算法的选项,删除多余代码优化体积
a3.3.0 被遗忘的版本号
a3.2 改进了算法，增快了运行速度,增加了可以修改算法的选项
a3.1 将原来的运行并保存按钮修改为是否保存的选项
